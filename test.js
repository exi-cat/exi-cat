...
let age = Number('хуй тебе, а не число');
alert(age);
...
if (startTest1 == true) {
    confirm('Тебе есть 18 лет, мелкий ты пиздюк?');

    let currentYear = new Date().getFullYear();
    let year = prompt('А ну введи год своего вылезания из мамки!');
        if(year < (`${currentYear - 18}`)) {
            confirm('А ты не пиздишь?');
            confirm('Точно?');
        }
            else {
                alert('Ну и вали от сюда, щегол!');
...
/* let startTest2 = confirm('Начать тесто № 2?');
    if (startTest2 == true) {
...
do {
    ...
    number = +prompt(`Вы ввели ${number}. А это меньше 100. Попробуйте ещё раз`,'');
} while (number < 100);
alert(`${number} подходящее число. Спасибо!`); */

//proverka работает ли iz d0ma

//ебём дальше дичь с простыми числами
// Я К ТЕБЕ ТВАРЬ ЕЩЁ ВЕРНУСЬ!!!!!!
...
// надо приебашить проверку на ввод чисел, пока не получается.
/* let user;
let text;
